...
public class AlreadyTaskFragment extends BaseFragment {
    @BindView(R.id.alreadyLV)
    PullToRefreshListView alreadyLV;
    Boolean judge = true;
    View view;
    int number;
    String status = "1";
    ...
    public int getContentViewId() {
        return R.layout.fragment_yaskbar_already;
    }
    ...
    protected void initViews(Bundle bundle) {
        ...
        alreadyLV.setAdapter(alreadyTaskAdapter);
        alreadyLV.setMode(PullToRefreshBase.Mode.BOTH);
        refresh();
        if (judge) {
            getTask();
            judge = false;
    ...
    private void commissionLVclick() {
        alreadyLV.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                JumpListItem.jumpListItem(getContext(), position, alreadyDatas);
            }
        });
        alreadyLV.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
    ...
    private void refresh() {
        alreadyLV.setOnRefreshListener(new PullToRefreshBase.OnRefreshListener2<ListView>() {
            @Override
